	...
	while{{Alive _X} count units _Group > 0} do {
		...
		_closePlayers = (_RandomUnit nearEntities [["Man"], 25]) select {(side _X != (side leader _Group)) && ((side leader _Group) getFriend (side _X) <= 0.6) && isPlayer _X && (side leader _Group) knowsAbout _X > 2.5};

...
Private _CasualtyRate = round ((_allPlayerDeaths / (count (allPlayers - entities "HeadlessClient_F"))) * 100);
private _ArrayOfDeathsAndNames = format ["Casualty Rate: %1",_CasualtyRate] + "%\n";
{
...
private _Color = "";
if(_allPlayerDeaths <= (count (allPlayers - entities "HeadlessClient_F") * 0.3)) then {
	_Color = "#24930C";
};
if(_allPlayerDeaths > (count (allPlayers - entities "HeadlessClient_F") * 0.3) && _allPlayerDeaths < (count (allPlayers - entities "HeadlessClient_F") * 0.6)) then {
	_Color = "#fcba03";
};
if(_allPlayerDeaths >= (count (allPlayers - entities "HeadlessClient_F") * 0.6)) then {
	_Color = "#ff0000";
};

...
OKS_Fastrope_Insert = {
	...
	//	null = [blufor,"",["spawn","PickUp","DropOff","despawn"],true] execVM "Scripts\NEKY_PickUp\OKS_Fastrope_Insert2.sqf";
	//	
	///////////////
	//	Examples:
	///////////////
	...
	_Crew setSpeedMode "LIMITED";
	WaitUntil {sleep 0.5; ( (!(Alive _Heli) or !(CanMove _Heli)) or (({Alive _x} count units _Crew) < 1) or ((_Heli distance _HeliPad) < 200))};
	_Players = ((fullCrew _Heli) select { isPlayer (_X select 0) }) apply {_X select 0};
	//_Heli flyInHeight 10;
	_Heli flyInHeightASL [10,10,10];
	WaitUntil {sleep 0.5; speed _Heli < 0.5 or (!(Alive _Heli) or !(CanMove _Heli)) or (({Alive _x} count units _Crew) < 1)};
	if ((Alive _Heli) && (Alive (driver _Heli))) then 
	{	
		WaitUntil {sleep 5; {((isNull objectParent _x) && (getPos _X select 2) < 0.5) || !(Alive _x)} Count _Players >= Count _Players};
		if ((!Alive _Heli) or (!Alive (driver _Heli)) or (!CanMove _Heli)) exitWith {DeleteVehicle _Helipad; False};
		_Heli DoMove (getpos _Heli);
		_Crew lockWP false;
		_Crew setSpeedMode "NORMAL";
...
OKS_SpawnFastRopeBird = {
	...
	if(isServer) then {
		[side _caller,_HeliClass,["helicopter_spawn",getPos reinsert_helipad,getPos NEKY_PARADROP_TRIGGER,"helicopter_despawn"],true,false] spawn OKS_Fastrope_Insert;
...
_flag addAction
[
	"<t color='#FFC31D'>* Request Fastrope *</t>",	// title
	{_this remoteExec ["OKS_SpawnFastRopeBird",2]},
